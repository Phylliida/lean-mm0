...
4. **General inductives + level equations.**  Generalise the Nat-primitive
   treatment to arbitrary inductives (à la `lean.mm1`'s `Ind`/`Ctor`/`Rec`),
   and add the `lvl` equations (`lmax`/`limax` laws) our verifier normalises.
5. **δ.**  MM0 statement-level `def` unfolding for our `def`s.
6. **Kernel integration.**  Drive the emitter from our `src/expr.py` CIC AST
